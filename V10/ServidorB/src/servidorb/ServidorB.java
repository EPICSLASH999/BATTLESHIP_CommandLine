...
public class ServidorB {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            // Ciclo para la aceptación de nuevas conexiones de clientes infinitamente
            ...
            for (String n : ServidorB.lista.keySet()) {
                if (nombre.equalsIgnoreCase(n)) {
                    salida.writeUTF("true"); // Manda que ya existe ese nombre
                    seguir = false;
                }
            }
            if (seguir) {
                salida.writeUTF("false"); // Manda que no existe ese nombre
            }

            String proseguir = entrada.readUTF(); // Recibe si se debe proseguir (de acuerdo si el nombre contiene espacios), si es "false" no se prosigue
            seguir = !proseguir.equals("false"); // Si proseguir no es "false", seguir = true

            if (!seguir) { // Si seguir = false, el ciclo se reinicia; sin continuar con lo de abajo
                continue;
            }
            // ----------------------------------------------------------------- //
